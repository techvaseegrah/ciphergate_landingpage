...
import { getAdminDeleteHistory, getDeletedInvoiceById } from '../../services/invoiceService';
...
const AdminDeleteHistory = () => {
  ...
  const fetchDeleteHistory = async () => {
    try {
      setLoading(true);
      const response = await getAdminDeleteHistory();
      if (response.success) {
  ...
  const formatDate = (dateString) => {
    ...
    const locale = settings?.localization?.locale || 'en-IN';
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 bg-white font-sans">
      <h1 className="text-xl font-bold text-gray-800 mb-6">Delete History</h1>
            ...
            <table className="min-w-full bg-white border border-gray-200">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="py-3 px-4 text-left text-sm font-semibold text-gray-700 border-b">Deleted By</th>
                  ...
                  <tr key={record._id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="py-3 px-4 text-sm text-gray-700 border-b">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        {record.deletedByRole}: {record.deletedByName}
                      </span>
                    </td>
